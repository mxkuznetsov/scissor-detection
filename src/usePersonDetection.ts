...
// YOLO class index for person (COCO dataset)
...
export const usePersonDetection = () => {
  const [detectedPersons, setDetectedPersons] = useState<PersonDetection[]>([]);
  ...
  // Process model output and extract person detections
  const processDetections = useCallback((
    ...
    for (let i = 0; i < numDetections; i++) {
      ...
      // Extract person class confidence (class 0 = person)
      const personConfidence = data[offset + 4 + PERSON_CLASS_INDEX];
      
      if (personConfidence >= YOLO_CONFIG.confidenceThreshold) {
    ...
    // Convert selected detections to PersonDetection format
